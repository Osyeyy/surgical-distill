...
Per-layer cosine similarity bar chart, saved as PNG.

Usage:
    python comparison/diagnostic/plot.py --results logs/diagnostic/diagnostic_results.json
"""
...
def plot_similarity(results: dict, output_dir: str):
    ...
    fig.suptitle(
        f"Baseline Divergence Map — Qwen2.5-3B (student) vs Qwen2.5-7B (teacher)\n"
        f"n={n_prompts} prompts | projection={'trained' if results['projection_trained'] else 'untrained (random init)'}",
        fontsize=12, fontweight="bold",
    )

    ax1 = axes[0]
    ax1.bar(student_layers, sim_values, color=colors, edgecolor="black", linewidth=0.5)
    ...
    ax1.set_xticklabels([f"s{i}\n→t{teacher_layers[i]}" for i in range(len(student_layers))], fontsize=7)
    ax1.set_ylim(0, 1)
    ...
    ax1.annotate(
        f"most divergent\ns{student_layers[min_idx]}→t{teacher_layers[min_idx]}",
    ...
    ax2.set_xticklabels([f"s{i}" for i in student_layers], fontsize=7)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results", type=str, default="logs/diagnostic/diagnostic_results.json")
